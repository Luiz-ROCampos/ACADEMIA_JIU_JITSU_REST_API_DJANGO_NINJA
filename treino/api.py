...
def criar_aluno(request, aluno_schema: AlunosSchema):
    ...
    if Alunos.objects.filter(email=email).exists() or Alunos.objects.filter(nome=nome):
...
def progresso_aluno(request, email_aluno: str):
    ...
    return {'email': aluno.email, 'nome': aluno.nome, 'faixa': faixa_atual, 'total_aulas': total_aulas_concluidas, 'aulas_necessarias_para_proxima_faixa': aulas_faltantes}

@treino_router.post('/aluna_realizada/', response={200: str})
def aula_realizada(request, aula_realizada: AulasRealizadasSchema):
    ...
    if qtd <= 0 :
        raise HttpError(400, 'Quantidade de aulas inválidas. Deve ser maior que 0.')
